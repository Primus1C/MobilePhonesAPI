...
Функция ВыгрузитьПоПравилу(Источник					= Неопределено,
	...
	Если ЕстьГлобальныйОбработчикПередКонвертациейОбъекта Тогда
		...
		Исключение
			...
			ИмяОбработчика = НСтр("ru = '%1 (глобальный)'; uk = '%1 (глобальний)'");
	...
	ИначеЕсли ФлагРежимОтладкиОбработчиков Тогда
		...
		Попытка
			...
			Если ЭтоВнешняяОбработка() Тогда
				...
				ВызватьИсключение
					НСтр("ru = 'Внешняя обработка отладки, загружаемая из файла на диске, не поддерживается.'; uk = 'Зовнішня обробка відладки, що завантажується з файлу на диску, не підтримується.'");
...
"Используется ANSI.'; uk = 'Невідоме кодування файлу протоколу обміну: %1."
		...
		Если АлгоритмыОбработчика.Количество() <> 0 Тогда // В этой строке есть вызовы алгоритмов.
			...
			Для Каждого Алгоритм Из АлгоритмыОбработчика Цикл
				...
				Если МассивЗапрошенныхЭлементов.Найти(ВРег(Алгоритм)) <> Неопределено Тогда // Рекурсивный вызов алгоритма.
					...
					СтрокаОператора = НСтр("ru = '%1 ""РЕКУРСИВНЫЙ ВЫЗОВ АЛГОРИТМА: %2"";'; uk = '%1 ""РЕКУРСИВНИЙ ВИКЛИК АЛГОРИТМУ: %2"";'");
...
Процедура ЗагрузитьПравилаОбмена(Источник="", ТипИсточника="XMLФайл") Экспорт
	...
	Если Конвертация.Свойство("ПослеЗагрузкиПравилОбмена", ТекстСобытияПослеЗагрузкиПравилОбмена)
		...
		Исключение
			...
			Текст = НСтр("ru = 'Обработчик: ""%1"": %2'; uk = 'Обробник: ""%1"": %2'");
...
Процедура ПроизвестиЧтениеДанных(РезультирующаяСтрокаСИнформациейОбОшибке = "") Экспорт
	...
	Исключение
		...
		СтрокаОшибки = ПодставитьПараметрыВСтроку(НСтр("ru = 'Ошибка при загрузке данных: %1'; uk = 'Помилка при завантаженні даних: %1'"), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ВыполнитьДействияПередЧтениемДанных(СтрокаДанных = "") Экспорт
	...
	Если Не ПустаяСтрока(Конвертация.ПередЗагрузкойДанных) Тогда
		...
		Исключение
			...
			ИмяОбработчика = НСтр("ru = '%1 (конвертация)'; uk = '%1 (конвертація)'"); 
...
Процедура ВыполнитьДействияПослеЗавершенияЧтенияДанных() Экспорт
	...
	Если ЭтоИнтерактивныйРежим Тогда
		СообщитьПользователю(НСтр("ru = 'Загрузка данных завершена.'; uk = 'Завантаження даних завершене.'"));
	КонецЕсли;
...
Процедура ВыполнитьВыгрузку() Экспорт
	...
	Если ЭтоИнтерактивныйРежим Тогда
		СообщитьПользователю(НСтр("ru = 'Выгрузка данных завершена.'; uk = 'Вивантаження даних завершене.'"));
	КонецЕсли;
...
Процедура ИнициализацияСообщений()
	...
	ТекстОшибки = НСтр("ru = 'Ошибка при создании COM-объекта %1'; uk = 'Помилка при створенні COM-об`єкта %1'");
	...
	одСообщения.Вставить(13, НСтр("ru = 'Ошибка получения значения свойства объекта (по имени свойства источника)'; uk = 'Помилка одержання значення властивості об`єкта (за ім`ям властивості джерела)'"));
	одСообщения.Вставить(14, НСтр("ru = 'Ошибка получения значения свойства объекта (по имени свойства приемника)'; uk = 'Помилка одержання значення властивості об`єкта (за ім`ям властивості приймача)'"));
	...
	одСообщения.Вставить(16, НСтр("ru = 'Ошибка получения значения свойства подчиненного объекта (по имени свойства источника)'; uk = 'Помилка одержання значення властивості підлеглого об`єкта (за ім`ям властивості джерела)'"));
	одСообщения.Вставить(17, НСтр("ru = 'Ошибка получения значения свойства подчиненного объекта (по имени свойства приемника)'; uk = 'Помилка одержання значення властивості підлеглого об`єкта (за ім`ям властивості приймача)'"));
	
	ТекстОшибки = НСтр("ru = 'Ошибка в обработчике события %1'; uk = 'Помилка в обробнику події %1'");
	...
	ТекстОшибки = НСтр("ru = 'Ошибка в обработчике события %1 (конвертация)'; uk = 'Помилка в обробнику події %1 (конвертація)'");
	...
	одСообщения.Вставить(45, НСтр("ru = 'Не найдено правило конвертации объектов'; uk = 'Не знайдено правило конвертації об`єктів'"));
	
	ТекстОшибки = НСтр("ru = 'Ошибка в обработчике события %1 группы свойств'; uk = 'Помилка в обробнику події %1 групи властивостей'");
	...
	ТекстОшибки = НСтр("ru = 'Ошибка в обработчике события %1 (объекта коллекции)'; uk = 'Помилка в обробнику події %1 (об`єкту колекції)'");
	...
	ТекстОшибки = НСтр("ru = 'Ошибка в глобальном обработчике события %1 (конвертация)'; uk = 'Помилка в глобальному обробнику події %1 (конвертація)'"); 
	...
	ТекстОшибки = НСтр("ru = 'Ошибка в обработчике события %1 (свойства)'; uk = 'Помилка в обробнику події %1 (властивості)'");
	...
	одСообщения.Вставить(77, НСтр("ru = 'Не указан файл внешней обработки с подключаемыми процедурами обработчиков событий'; uk = 'Не зазначений файл зовнішньої обробки процедурами оброблювачів подій, що підключаються'"));
...
Функция ОпределитьДостаточностьПараметровДляПодключенияКИнформационнойБазе(СтруктураПодключения, СтрокаПодключения = "", СтрокаСообщенияОбОшибке = "")
	...
	Иначе
		...
		Если ПустаяСтрока(СтруктураПодключения.ИмяСервера) Тогда
			...
			СтрокаСообщенияОбОшибке = НСтр("ru = 'Не задано имя сервера платформы информационной базы-приемника'; uk = 'Не задане ім`я сервера платформи інформаційної бази-приймача'");
		...
		Если ПустаяСтрока(СтруктураПодключения.ИмяИБНаСервере) Тогда
			...
			СтрокаСообщенияОбОшибке = НСтр("ru = 'Не задано имя информационной базы-приемника на сервере платформы'; uk = 'Не задане ім`я інформаційної бази-приймача на сервері платформи'");
...
Функция ВыполнитьПодключениеКИБПриемнику() Экспорт
	...
	Исключение
		...
		Текст = НСтр("ru = 'При попытке создания обработки %1 произошла ошибка: %2'; uk = 'При спробі створення обробки %1 сталася помилка: %2'");
...
// © Primus1C  19.10.2023  НАЧАЛО ►

Функция _СтрНайти(Строка, ПодстрокаПоиска)

	Возврат	Найти(Строка, ПодстрокаПоиска)

КонецФункции // ()    

Функция _СтрНачинаетсяС(Строка, ПодстрокаПоиска)

	Возврат Лев(Строка, СтрДлина(ПодстрокаПоиска)) = ПодстрокаПоиска	

КонецФункции // СтрНачинаетсяС()

Функция _СтрРазделить(Строка, Разделитель, ВключатьПустые)

	Возврат ОбщегоНазначения.СтрРазделить_(Строка, Разделитель);	

КонецФункции // СтрРазделить(<Строка>, <Разделитель>, <ВключатьПустые>)()
...
#Иначе
ВызватьИсключение НСтр("ru = 'Недопустимый вызов объекта на клиенте.'; uk = 'Неприпустимий виклик об`єкту на клієнті.'");
#КонецЕсли

...
Процедура ОбновитьНастройки()
	...
	Для Каждого ГруппаНастроек Из НастройкиГруппы Цикл
		...
		Если ГруппаНастроек.ЭтоГруппа Тогда
		...
		Иначе
			...
			Запись.Пользователь = Пользователь;
			Запись.Настройка    = ГруппаНастроек.Настройка;
			Запись.Значение     = ГруппаНастроек.Настройка.ТипЗначения.ПривестиЗначение(ГруппаНастроек.Значение);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтильОформленияСообщений = ПланыВидовХарактеристик.НастройкиПользователей.СтильОформленияСообщений;
	
	Если Параметры.Свойство("Пользователь") Тогда
		
		Пользователь = Параметры.Пользователь;
		
		Заголовок = Строка(Пользователь);

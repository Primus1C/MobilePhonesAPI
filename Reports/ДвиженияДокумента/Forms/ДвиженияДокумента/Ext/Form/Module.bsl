...
Процедура ОбработатьВыводДанныхПоМассиву(СписокПолей, ТаблицаРесурсов, ТаблицаИзмерений, ТаблицаРеквизитов, ТаблицаВидДвижений = Неопределено, Знач ИмяРегистра, СинонимРегистра)
	...
	//ЫЫЫКодЯзыкаПечать = Локализация.ПолучитьЯзыкФормированияПечатныхФорм();	
	КодЯзыкаПечать = "ru";
	...
	Если Отчет.СпособВыводаОтчета = Перечисления.СпособВыводаОтчета.ПоГоризонтали Тогда
		...
		Если ТаблицаВидДвижений <> Неопределено Тогда
			ОбластьЗаголовокЯчейки.Параметры.ЗаголовокКолонки = НСтр("ru='Вид движения';uk='Вид руху';en='Type of movement'", КодЯзыкаПечать);
...
Процедура ВывестиЖурналПроводок()
	...
	|	Управленческий.НашМагазин КАК Организация,
	|	Управленческий.НомерЖурнала КАК ПериодПланирования,
	|	Управленческий.ВалютаДт КАК ВалютаДт,
	|	Управленческий.ВалютаКт КАК ВалютаКт,
	|	Управленческий.Сумма КАК Сумма,
	|	Управленческий.ВалютнаяСуммаДт КАК СуммаВалДт,
	|	Управленческий.ВалютнаяСуммаКт КАК СуммаВалКт,
	|	Управленческий.Комментарий КАК Содержание
	|ИЗ
	|	РегистрБухгалтерии.РегистрДляПланаСчетов_Оптовый КАК Управленческий
	|ГДЕ
...
Процедура СформироватьОтчет()
    ...
    Если НЕ ЗначениеЗаполнено(Отчет.Документ) Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Не выбран документ!'; en = 'No document selected!'");       
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
	...
	Для Каждого СвойстваОбъекта Из ДвиженияДокумента Цикл
		...
		СинонимРегистра = НСтр("ru = 'Регистр '; en = 'Register '") + НРег(ВидРегистра) + " """ + СвойстваОбъекта.Синоним + """";
...
&НаКлиенте
Процедура Проводки(Команда)
	Тек = Отчет.Документ;
	Если гоПЗ(Тек) Тогда Возврат КонецЕсли; 
	
	Отбор = Новый Структура("Регистратор", Тек);
	ПараметрыФормы = Новый Структура("Отбор, СформироватьПриОткрытии, КлючВарианта", Отбор, Истина, "Основной");
	ОткрытьФорму("Отчет.ПроводкиДокумента.Форма", ПараметрыФормы, ЭтаФорма, Истина, );
КонецПроцедуры

&НаКлиенте
Процедура БезВалют(Команда)
	Тек = Отчет.Документ;
	Если гоПЗ(Тек) Тогда Возврат КонецЕсли; 
	
	Отбор = Новый Структура("Регистратор", Тек);
	ПараметрыФормы = Новый Структура("Отбор, СформироватьПриОткрытии, КлючВарианта", Отбор, Истина, "БезВалют");
	ОткрытьФорму("Отчет.ПроводкиДокумента.Форма", ПараметрыФормы, ЭтаФорма, Истина, );
КонецПроцедуры

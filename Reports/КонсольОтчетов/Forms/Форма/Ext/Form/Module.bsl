...
#Иначе
	ПоказатьПредупреждение(Неопределено, НСтр("ru='Конструктор схемы компоновки данных можно открыть только в толстом клиенте. В тонком клиенте и веб клиенте редактирование схемы компоновки данных возможно только в тексте схемы компоновки данных.'"));
#КонецЕсли
...
&НаКлиенте
Процедура ПодтвердитьЗакрытие(ОписаниеОповещенияОЗавершении)
	...
	Если Модифицированность Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ПодтвердитьЗакрытиеПослеВопроса", ЭтотОбъект, ОписаниеОповещенияОЗавершении), 
			НСтр("ru='Отчеты модифицированы. Сохранить изменения?'"), РежимДиалогаВопрос.ДаНетОтмена, , КодВозвратаДиалога.Да);
...
&НаКлиенте
Процедура ПодтвердитьЗакрытиеПослеВопроса(Ответ, ОписаниеОповещенияОЗавершении) Экспорт
	
	Если Ответ = КодВозвратаДиалога.Да Тогда
		
		ВыполнитьОбработкуОповещения(ОписаниеОповещенияОЗавершении, Сохранить(Ложь));
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если ОтветПередЗакрытием <> Истина Тогда
		Отказ = Истина;
		ПодтвердитьЗакрытие(Новый ОписаниеОповещения("ПередЗакрытиемПослеПодтверждения", ЭтотОбъект));
	КонецЕсли;
...
Процедура ДобавитьПользовательскиеНастройки(Команда)
	...
	Если ИскомаяСхемаКомпоновкиДанныхИлиВариант = Неопределено Тогда
		...
		ИначеЕсли ВариантыОтчета.Количество() > 1 Тогда
			...
			ПоказатьПредупреждение(Неопределено, НСтр("ru='Программа не может определить, какому варианту отчета нужно добавить настройку. Выберите отчет или вариант отчета, которому нужно добавить настройку, после чего повторите добавление настройки.'"));
			Возврат;
		...
		ИначеЕсли СхемыКомпоновки.Количество() = 0 Тогда
			...
			ПоказатьПредупреждение(Неопределено, НСтр("ru='В файле нет ни одного отчета. Создайте отчет, после чего повторите добавление варианта отчета.'"));
			Возврат;
...
Процедура ДобавитьВариантОтчета(Команда)
	...
	Если ИскомаяСхемаКомпоновкиДанных = Неопределено Тогда
		...
		Иначе
			...
			ПоказатьПредупреждение(Неопределено, НСтр("ru='Программа не может определить, какому отчету нужно добавить вариант. Выберите отчет, которому нужно добавить вариант, после чего повторите добавление варианта.'"));
			Возврат;
...
Процедура ОткрытьФайлОтчетов(Команда)
	...
	ПодтвердитьЗакрытие(Новый ОписаниеОповещения("ОткрытьФайлОтчетовПослеПодтверждения", ЭтотОбъект));
	
КонецПроцедуры

&НаКлиенте
Процедура ОткрытьФайлОтчетовПослеПодтверждения(Результат, ДопПараметры) Экспорт
...
Процедура НовыйФайлОтчетов(Команда)
	...
	ПодтвердитьЗакрытие(Новый ОписаниеОповещения("НовыйФайлОтчетовПослеПодтверждения", ЭтотОбъект));
	
КонецПроцедуры

&НаКлиенте
Процедура НовыйФайлОтчетовПослеПодтверждения(Результат, ДопПараметры) Экспорт
...
#Иначе	
	ПоказатьПредупреждение(Неопределено, НСтр("ru='Сравнение файлов возможно только в толстом клиенте'"));
#КонецЕсли
